...
<head>
    <meta charset="utf-8" />
    <title>Purchase Details #PUR-{{ str_pad($purchase->id, 5, '0', STR_PAD_LEFT) }}</title>
    ...
    <div class="full-window-modal">
        ...
        <div class="modal-header">
            <h4 class="mb-0 fw-bold d-flex align-items-center">
                ...
                Purchase Details #PUR-{{ str_pad($purchase->id, 5, '0', STR_PAD_LEFT) }}
            </h4>
            <div class="d-flex align-items-center gap-2">
                <button class="btn btn-secondary d-print-none" onclick="window.print()"><i class="ri-printer-line align-middle me-1"></i> Print</button>
                    ...
                    <div class="col-lg-12">
                        <div class="card" id="demo">
                            ...
                            <div class="card-body p-4">
                                <h1 class="text-center mb-4 mt-2 text-uppercase" style="font-size: 2.0rem; font-weight: 800; letter-spacing: 1px;">Purchase Invoice</h1>
                                
                                <div class="row mb-4">
                                    <div class="col-4 text-center border-end">
                                        <h6 class="text-muted text-uppercase fw-semibold mb-1">Invoice #</h6>
                                        <p class="fs-15 mb-0">PUR-{{ str_pad($purchase->id, 5, '0', STR_PAD_LEFT) }}</p>
                                    </div>
                                    <div class="col-4 text-center border-end">
                                        <h6 class="text-muted text-uppercase fw-semibold mb-1">Vendor</h6>
                                        <p class="fs-15 mb-0 fw-bold">{{ $purchase->vendor->name ?? 'N/A' }}</p>
                                    </div>
                                    <div class="col-4 text-center">
                                        <h6 class="text-muted text-uppercase fw-semibold mb-1">Date</h6>
                                        <p class="fs-15 mb-0">{{ \Carbon\Carbon::parse($purchase->transaction_date ?? $purchase->created_at)->format('d M, Y') }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="row justify-content-end">
                                    <div class="col-lg-5 col-md-6 col-sm-8">
                                        @php
                                            $net_amount = $purchase->net_amount ?? $purchase->total_amount;
                                            $paid_amount = $purchase->paid_amount ?? 0;
                                            $pending = max(0, $net_amount - $paid_amount);
                                            $previousBalance = $purchase->vendor ? $purchase->vendor->balance - $pending : 0;
                                        @endphp
                                        <div class="d-flex justify-content-between mb-2">
                                            <span class="text-muted">Discount:</span>
                                            <span class="text-danger">- {{ number_format($purchase->discount ?? 0, 2) }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between mb-2">
                                            <span class="text-muted">Delivery:</span>
                                            <span>+ {{ number_format($purchase->delivery_charges ?? 0, 2) }}</span>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between mb-2">
                                            ...
                                            <span class="text-success">{{ number_format($paid_amount, 2) }}</span>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between bg-light p-2 rounded mt-2 fs-15 fw-bold text-primary">
                                            ...
                                            <span>{{ number_format($purchase->vendor->balance, 2) }}</span>
                                        </div>
                                        @endif
                                    </div>
                                </div>
